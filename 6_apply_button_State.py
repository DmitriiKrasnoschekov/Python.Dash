...
@app.callback(
    ...
    [Input(component_id = 'submit-val', component_property = 'n_clicks')],    
    [State(component_id = 'range-slider', component_property = 'value'),
    State(component_id = 'star-selector', component_property = 'value')]  # устанавливаем смену фильтров только после кнопки
)
def update_dist_celestial_chart(n, radius_range, star_size):
    chart_data = df[(df['RPLANET'] > radius_range[0]) &
                    (df['RPLANET'] < radius_range[1]) &
                    (df['StarSize'].isin(star_size))]
